...
const Manage = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get('/api/school-year/'); // Thay thế your-endpoint bằng đường dẫn API của bạn
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={{ width: "100%", height: 50, justifyContent: "center", alignItems: "center", backgroundColor: 'rgba(192,192,192,0.7)' }}>
        <Text style={{ fontSize: 20, fontWeight: 'bold', }}>Năm học</Text>
      </View>
        ...
        <ScrollView style={styles.container}>
          ...
          <View style={styles.table}>
            <View style={styles.headerRow}>
              ...
              <Text style={[styles.headerItem, styles.column2]}>Năm học</Text>
            </View>
            {/* Sử dụng data để render dữ liệu */}
            {data && data.map((item, index) => (
              <View key={index} style={styles.row}>
                <Text style={[styles.column1]}>{index + 1}</Text>
                <Text style={[styles.column2]}>{`${item.fromYear} - ${item.toYear}`}</Text>
              </View>
            ))}
          </View>
        </ScrollView>
